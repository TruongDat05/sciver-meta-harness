...
def test_injected_boundary_rejects_invalid_client_result(tmp_path):
    ...
    with pytest.raises(SolverError, match="SolverResult"):
...
def test_locked_model_preflight_never_selects_an_arbitrary_model():
    ...
    with pytest.raises(SolverError, match="immutable solver"):
...
sys.modules.pop("meta_harness.solver", None)
with patch.object(
    remote_client,
    "RemoteChatCompletionsClient",
    ...
    importlib.import_module("meta_harness.solver")
"""
