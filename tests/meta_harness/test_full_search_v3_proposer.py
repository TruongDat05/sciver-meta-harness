"""Offline contract tests for the one-candidate full SEARCH V3 proposer."""
...
def _propose(tmp_path, runner, **kwargs):
    return FullSearchV3Proposer(runner=runner).propose(
...
def test_missing_or_extra_template_keys_are_rejected_before_any_solver_call(
    ...
    with pytest.raises(FullSearchV3ProposalExhausted):
