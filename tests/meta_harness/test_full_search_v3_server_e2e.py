...
from meta_harness.full_search_v3_final import FullSearchV3FinalError
...
def _p0(**_kwargs):
    return _report("cot", canonical_full_search_v3_p0_prompt_sha256(), 0.9)
...
def test_server_interface_end_to_end_with_fake_boundaries(
    ...
    prepared = server.prepare_full_search_v3_server_run(
    ...
    monkeypatch.setattr(server, "FullSearchV3Proposer", lambda **_kwargs: proposer)
    ...
    monkeypatch.setattr(server, "FullSearchV3Orchestrator", engine)
    live_clients = []
    monkeypatch.setattr(
        server,
    ...
    with pytest.raises(server.FullSearchV3ServerAuthorizationError):
    ...
    preflight = server.preflight_full_search_v3_server_run(
        repository_root=tmp_path,
        run_id=RUN_ID,
        search_safe_manifest_path=prepared["search_safe_manifest_path"],
        search_records_path=prepared["search_dataset_path"],
    ...
    with pytest.raises(FullSearchV3FinalError, match="frozen winner"):
        server.preflight_full_search_v3_server_final(
            repository_root=tmp_path,
            run_id=RUN_ID,
            dataset_path=synthetic_source,
            private_manifest_path=prepared["private_manifest_path"],
    ...
    assert server.inspect_full_search_v3_server_status(
    ...
    frozen = server.freeze_full_search_v3_server_winner(repository_root=tmp_path, run_id=RUN_ID)
    final_preflight = server.preflight_full_search_v3_server_final(
        repository_root=tmp_path,
        run_id=RUN_ID,
        dataset_path=synthetic_source,
        private_manifest_path=prepared["private_manifest_path"],
    ...
    generation = SolverGenerationSettings.from_config(canonical_full_search_v3_config())
    monkeypatch.setattr(
        final_module,
    ...
    monkeypatch.setattr(server, "_construct_live_solver", lambda *_args: resumed)
    final = server.start_or_resume_full_search_v3_server_final(
        repository_root=tmp_path,
        run_id=RUN_ID,
        dataset_path=synthetic_source,
        private_manifest_path=prepared["private_manifest_path"],
    ...
    assert len(set(interrupted.calls + resumed.calls)) == 2000
