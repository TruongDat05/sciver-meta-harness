...
def split(validated_records):
    return build_experiment_split(validated_records)


@pytest.fixture(scope="module")
def private_manifest(split):
    return build_experiment_private_manifest(
        split,
...
def test_private_manifest_binds_exact_v3_split_and_rejects_conflicts(
    ...
    assert save_experiment_private_manifest(path, private_manifest) == path
    loaded = load_trusted_experiment_private_manifest(path)
    verify_experiment_private_manifest(
        loaded,
    ...
    assert loaded["schema_version"] == EXPERIMENT_PRIVATE_MANIFEST_SCHEMA
    ...
    conflicting = build_experiment_private_manifest(
        loaded["split"],
        source_dataset_sha256="0" * 64,
    )
    with pytest.raises(PreparationError, match="refusing to replace"):
        save_experiment_private_manifest(path, conflicting)
...
def test_search_safe_manifest_is_opaque_about_final_membership(private_manifest):
    ...
    assert safe["schema_version"] == EXPERIMENT_SEARCH_SAFE_MANIFEST_SCHEMA
    ...
    verify_experiment_search_safe_manifest(safe, private_manifest)

    changed_split = copy.deepcopy(private_manifest["split"])
    changed_split["FINAL"]["sample_ids"].reverse()
    assert compute_final_evaluation_commitment(changed_split) != safe[
        "final_membership_commitment"
    ]
...
def test_final_commitment_binds_split_identity(private_manifest):
    ...
    assert EXPERIMENT_FINAL_COMMITMENT_SCHEMA.startswith(
        "sciver_full_search_v3_"
    )
    assert compute_final_evaluation_commitment(changed_split) != private_manifest[
        "final_membership_commitment"
    ]
...
def test_materialized_search_dataset_is_manifest_ordered_and_evidence_ordered(
    ...
    safe = derive_experiment_search_safe_manifest(private_manifest)
    records = materialize_experiment_search_records(
        validated_records,
        private_manifest,
        source_path=TESTSET_PATH,
    )
    ...
    verify_experiment_search_dataset(records, safe)
...
def test_preparation_is_deterministic_and_binds_source_content(tmp_path):
    ...
    first_private = load_trusted_experiment_private_manifest(
        first.private_manifest_path
    )
    second_private = load_trusted_experiment_private_manifest(
        second.private_manifest_path
    )
    first_safe = load_experiment_search_safe_manifest(
        first.search_safe_manifest_path
    )
    second_safe = load_experiment_search_safe_manifest(
        second.search_safe_manifest_path
    )
    first_records = load_experiment_search_dataset(first.search_dataset_path)
    second_records = load_experiment_search_dataset(second.search_dataset_path)
...
def test_reordered_source_has_identical_split_and_commitment(
    ...
    reordered_records = validate_sciver_records(
        list(reversed(source_records)),
        source_path=TESTSET_PATH,
    )
    reordered_split = build_experiment_split(reordered_records)
    ...
    assert compute_final_evaluation_commitment(
        reordered_split
    ) == compute_final_evaluation_commitment(split)
...
def test_full_preparation_is_input_order_independent(tmp_path, source_records):
    ...
    original_private = load_trusted_experiment_private_manifest(
        original.private_manifest_path
    )
    reordered_private = load_trusted_experiment_private_manifest(
        reordered.private_manifest_path
    )
    original_safe = load_experiment_search_safe_manifest(
        original.search_safe_manifest_path
    )
    reordered_safe = load_experiment_search_safe_manifest(
...
def test_search_safe_loader_never_returns_private_final_membership(
    ...
    save_experiment_search_safe_manifest(path, safe)
    loaded = load_experiment_search_safe_manifest(path)
...
def test_search_dataset_verification_rejects_missing_duplicate_and_reordered_rows(
    ...
    with pytest.raises(PreparationError, match="count"):
        verify_experiment_search_dataset(records[:-1], safe)
    duplicate = [*records[:-1], records[0]]
    with pytest.raises(PreparationError, match="duplicate"):
        verify_experiment_search_dataset(duplicate, safe)
    reordered = list(reversed(records))
    with pytest.raises(PreparationError, match="order"):
        verify_experiment_search_dataset(reordered, safe)
