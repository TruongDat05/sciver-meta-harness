"""Compact offline regressions for full-SEARCH server-readiness invariants."""
...
import meta_harness.search_evaluator as evaluator
...
def production_search_input(tmp_path_factory):
    root = tmp_path_factory.mktemp("meta-harness-critical")
...
def test_production_input_requires_exact_unique_manifest_order(
    ...
    with pytest.raises(evaluator.EvaluatorInputError, match="1,000"):
        validate_experiment_search_input(
            manifest=manifest,
            records=records[:-1],
        )
    ...
    with pytest.raises(evaluator.EvaluatorInputError, match="duplicate"):
        validate_experiment_search_input(manifest=manifest, records=duplicate)

    with pytest.raises(
        evaluator.EvaluatorInputError, match="immutable manifest order"
    ):
        validate_experiment_search_input(
            manifest=manifest,
...
def test_p0_and_candidate_share_frozen_request_parser_and_image_path(tmp_path):
    ...
    p0_requests = evaluator._candidate_requests(
        ...
        candidate_id="cot",
        prompt=COT_PROMPT,
        prompt_sha256=evaluator.canonical_experiment_p0_prompt_sha256(),
    ...
    assert evaluator.canonical_experiment_p0_prompt_sha256() == evaluator.template_source_sha256(COT_PROMPT)
    ...
    outcomes = tuple(
        PredictionOutcome(sample_id, "Answer: yes")
...
def test_metrics_are_trusted_and_hand_computable():
    ...
    outcomes = tuple(
        PredictionOutcome(sample_id, f"Answer: {prediction}")
...
def test_evaluator_rejects_a_mismatched_m2_cache_before_dispatch(
    ...
    evaluator_cache = SearchCache(tmp_path / "evaluator-cache")
    executor_cache = SearchCache(tmp_path / "executor-cache")
    client = _FakeSolver("Answer: yes")

    with pytest.raises(
        evaluator.EvaluatorInputError, match="same SEARCH cache"
    ):
        evaluator.evaluate_experiment_p0(
...
def test_search_interface_has_no_private_final_input(production_search_input):
    ...
    assert "load_experiment_private" not in source
    assert "FINAL" not in production_search_input.manifest
    assert all(
        "final" not in parameter.lower() and "private" not in parameter.lower()
        for function in (
            evaluator.load_experiment_search_input,
            evaluator.validate_experiment_search_input,
            evaluator.evaluate_experiment_candidate,
            evaluator.evaluate_experiment_p0,
        )
