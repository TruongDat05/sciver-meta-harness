...
import meta_harness
import meta_harness.full_search_v3_evaluator as full_search_v3_evaluator
...
def prepared_search_input(tmp_path_factory):
    root = tmp_path_factory.mktemp("full-search-v3-evaluator")
...
def test_loads_only_complete_required_manifest_ordered_search_input(
    ...
    assert loaded_search_input.stage == FULL_SEARCH_V3_STAGE
...
def test_invalid_input_is_rejected_before_any_solver_dispatch(
    ...
    with pytest.raises(FullSearchV3EvaluatorInputError, match=message):
        validated = validate_full_search_v3_search_input(
            manifest=manifest,
            records=records,
        )
...
def test_loader_rejects_mismatched_artifact_before_any_solver_dispatch(
    ...
    with pytest.raises(FullSearchV3EvaluatorInputError):
        loaded = load_full_search_v3_search_input(
...
def test_unvalidated_public_input_never_persists_or_dispatches(
    ...
    records = _mutable(loaded_search_input.records)
    mutate(manifest, records)
    unvalidated = FullSearchV3SearchInput(
        _manifest=manifest,
    ...
    cache = FullSearchV3SearchCache(cache_path)
...
def test_raw_metrics_writer_is_private_and_forged_mapping_has_no_public_path(tmp_path):
    ...
    writer_name = "persist_full_search_v3_prediction_metrics"
    forged_path = tmp_path / "forged-metrics.json"
    forged_report = {"metrics": {"rankable": True}}

    assert writer_name not in full_search_v3_evaluator.__all__
    assert not hasattr(full_search_v3_evaluator, writer_name)
    assert not hasattr(meta_harness, writer_name)
    with pytest.raises(AttributeError):
        getattr(full_search_v3_evaluator, writer_name)(forged_path, forged_report)
    assert not forged_path.exists()

    public_result_paths = {
        name: set(inspect.signature(getattr(full_search_v3_evaluator, name)).parameters)
        for name in full_search_v3_evaluator.__all__
        if inspect.isfunction(getattr(full_search_v3_evaluator, name))
        and {"metrics_path", "result_path"}
        & set(inspect.signature(getattr(full_search_v3_evaluator, name)).parameters)
    }
...
def test_incomplete_outcomes_cannot_persist_rankable_metrics(
    ...
    incomplete_outcomes = tuple(
        FullSearchV3PredictionOutcome(sample_id, raw_response="Answer: yes")
        for sample_id in loaded_search_input.sample_ids[:-1]
    )

    with pytest.raises(FullSearchV3EvaluatorInputError, match="cover every"):
        account_full_search_v3_predictions(
...
def test_unsafe_candidate_id_never_persists_or_dispatches(
    ...
    with pytest.raises(FullSearchV3EvaluatorInputError, match="safe opaque"):
        account_full_search_v3_predictions(
            search_input=loaded_search_input,
...
def test_pure_prediction_metrics_perfect_and_all_wrong_are_hand_computable():
    ...
    perfect = full_search_v3_evaluator._compute_prediction_metrics(
        search_input,
        _outcomes(search_input, ["yes", "no", "yes", "no"]),
    )
    wrong = full_search_v3_evaluator._compute_prediction_metrics(
        search_input,
...
def test_pure_prediction_metrics_class_imbalance_and_one_class_predictions():
    ...
    imbalanced_report = full_search_v3_evaluator._compute_prediction_metrics(
        imbalanced,
        _outcomes(imbalanced, ["yes", "yes", "no", "no"]),
    )
    one_class = _synthetic_search_input(["yes", "no", "no"])
    one_class_report = full_search_v3_evaluator._compute_prediction_metrics(
        one_class,
...
def test_abstention_and_parse_failure_remain_in_full_metric_denominator():
    ...
    abstention = full_search_v3_evaluator._compute_prediction_metrics(
        search_input,
        _outcomes(search_input, ["yes", "I cannot determine."]),
    )
    parse_failure = full_search_v3_evaluator._compute_prediction_metrics(
        search_input,
        _outcomes(
            search_input,
...
def test_pure_metrics_infrastructure_failure_is_not_a_prediction(tmp_path):
    ...
    outcome = FullSearchV3PredictionOutcome.from_infrastructure_failure(
        search_input.sample_ids[1], SolverExecutionFailure(metadata)
    )
    report = full_search_v3_evaluator._compute_prediction_metrics(
        search_input,
        (
            FullSearchV3PredictionOutcome(
...
def test_full_search_checkpoint_resume_is_atomic_deterministic_and_noops_completed(
    ...
    monkeypatch.setattr(
        "meta_harness.full_search_v3_evaluator._candidate_requests",
    ...
    assert interrupted_payload["schema_version"] == FULL_SEARCH_V3_CHECKPOINT_SCHEMA_VERSION
    ...
    report = evaluate_full_search_v3_p0(
        search_input=loaded_search_input,
        solver_identity_sha256="1" * 64,
        cache=cache,
    ...
    assert report["prompt_sha256"] == canonical_full_search_v3_p0_prompt_sha256()
    ...
    resumed_report = evaluate_full_search_v3_p0(
        search_input=loaded_search_input,
        solver_identity_sha256="1" * 64,
        cache=cache,
...
def test_resume_rejects_incompatible_and_corrupt_checkpoint_before_solver(
    ...
    with pytest.raises(FullSearchV3EvaluationResumeError, match="corrupt"):
        evaluate_full_search_v3_candidate(
            search_input=loaded_search_input,
            candidate_id="p0",
            prompt=COT_PROMPT,
    ...
    with pytest.raises(FullSearchV3EvaluationResumeError, match="incompatible"):
        evaluate_full_search_v3_candidate(
            search_input=loaded_search_input,
            candidate_id="p0",
            prompt=COT_PROMPT,
...
def test_infrastructure_incomplete_checkpoint_never_persists_eligible_result(
    ...
    with pytest.raises(FullSearchV3EvaluationIncomplete) as raised:
        evaluate_full_search_v3_candidate(
            search_input=loaded_search_input,
            candidate_id="p0",
            prompt=COT_PROMPT,
...
def test_public_evaluator_rejects_non_full_input_before_solver(
    ...
    cache = FullSearchV3SearchCache(tmp_path / "cache")
    client = _CountingClient()
    shortened = FullSearchV3SearchInput(
        _manifest=loaded_search_input.manifest,
        _records=loaded_search_input.records[:2],
    )

    with pytest.raises(FullSearchV3EvaluatorInputError, match="exactly 1,000"):
        evaluate_full_search_v3_candidate(
...
def test_p0_request_path_is_exactly_canonical_and_preserves_multimodal_order(
    ...
    p0_hash = canonical_full_search_v3_p0_prompt_sha256()
    ...
    full_search_requests = full_search_v3_evaluator._candidate_requests(
    ...
    for item, record, paths in zip(
        ...
        assert identity.candidate_id == FULL_SEARCH_V3_P0_CANDIDATE_ID
...
def test_p0_parser_labels_and_metrics_match_existing_canonical_boundary(tmp_path):
    ...
    outcomes = tuple(
        FullSearchV3PredictionOutcome(sample_id, raw_response=response)
        for sample_id, response in zip(search_input.sample_ids, raw_responses)
    )
    full_search_records = full_search_v3_evaluator._metric_records(
        search_input,
        FULL_SEARCH_V3_P0_CANDIDATE_ID,
        outcomes,
    )
        ...
        canonical_records.append(
            {
                ...
                "prompt_variant": FULL_SEARCH_V3_P0_CANDIDATE_ID,
...
def test_p0_entrypoint_uses_frozen_mapping_and_validates_its_hash(monkeypatch):
    captured = {}
    expected_hash = canonical_full_search_v3_p0_prompt_sha256()
    ...
    monkeypatch.setattr(
        full_search_v3_evaluator,
        "evaluate_full_search_v3_candidate",
        canonical_result,
    )
    result = evaluate_full_search_v3_p0(
        search_input=object(),
    ...
    assert captured["candidate_id"] == FULL_SEARCH_V3_P0_CANDIDATE_ID
    ...
    with pytest.raises(FullSearchV3EvaluatorInputError, match="frozen COT prompt hash"):
        evaluate_full_search_v3_p0(
            search_input=object(),
            solver_identity_sha256="3" * 64,
            cache=object(),
...
def test_p0_dispatches_original_cot_mapping_to_request_builder(
    ...
    assert captured["prompt"] is COT_PROMPT
    assert captured["prompt_sha256"] == canonical_full_search_v3_p0_prompt_sha256()
    assert client.calls == 0


def test_full_search_v3_offline_1000_record_integration(
    tmp_path,
    monkeypatch,
):
    ...
    resumed_input = load_full_search_v3_search_input(
    ...
    p0_report = evaluate_full_search_v3_p0(
    ...
    candidate_a_report = evaluate_full_search_v3_candidate(
        search_input=resumed_input,
    ...
    with pytest.raises(FullSearchV3EvaluationIncomplete) as incomplete:
        evaluate_full_search_v3_candidate(
            search_input=resumed_input,
...
def test_full_search_v3_evaluator_has_no_legacy_stage_or_final_dependencies():
    """The dedicated evaluator must not import or call legacy execution paths."""

    source = Path(full_search_v3_evaluator.__file__).read_text(encoding="utf-8")
...
def _synthetic_1000_search_artifact(tmp_path):
    ...
    manifest = {
        ...
        "preparation_identity_schema_version": FULL_SEARCH_V3_PREPARATION_IDENTITY_SCHEMA,
