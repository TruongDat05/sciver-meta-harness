"""Deterministic offline end-to-end coverage for full-SEARCH V3 Milestone 5."""
...
class _SearchEvaluator:
    ...
    def p0(self, **_kwargs):
        from meta_harness.full_search_v3_evaluator import canonical_full_search_v3_p0_prompt_sha256

        return self._report("cot", canonical_full_search_v3_p0_prompt_sha256(), self.p0_metrics)
...
class _FakeFinalSolver:
    ...
    def complete(self, request):
        ...
        assert "GOLD_LABEL_SENTINEL" not in serialized
...
def synthetic_preparation(tmp_path_factory):
    ...
    private = load_trusted_full_search_v3_private_manifest(artifacts.private_manifest_path)
    checked = validate_sciver_full_search_v3_records(rows, source_path=source)
    adapted = get_dataset_adapter("SciVer").load(source)
    normalized = {
        record.sample_id: {
            **item.record,
            "sample_id": record.sample_id,
            "gold_label": "GOLD_LABEL_SENTINEL",
        }
...
def test_prepare_search_freeze_paired_final_resume_and_isolation(
    ...
    search = FullSearchV3Orchestrator(
    ...
    with monkeypatch.context() as guarded:
        ...
        with pytest.raises(Exception):
            preflight_full_search_v3_final(
                ...
                solver_contract=canonical_full_search_v3_final_solver_contract(solver_identity_sha256="c" * 64),
            )

    frozen = freeze_full_search_v3_winner(repository_root=repository_root, run_id=run_id)
    frozen_path = repository_root / "workspace" / "meta_harness" / "full_search_v3" / run_id / "freeze" / "frozen_winner.json"
    frozen_bytes = frozen_path.read_bytes()
    contract = canonical_full_search_v3_final_solver_contract(solver_identity_sha256="c" * 64)
    ...
    assert full_search_v3_final_completion_receipt_path(repository_root, run_id).is_file()
    final_state = load_full_search_v3_final_state(
    ...
    assert all("GOLD_LABEL_SENTINEL" not in item["messages"] for item in all_successful)
