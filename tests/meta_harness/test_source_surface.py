...
CURRENT_MODULES = (
    ...
    "meta_harness.records",
    "meta_harness.preparation",
    "meta_harness.solver",
    "meta_harness.search_cache",
    "meta_harness.retry",
    "meta_harness.request_executor",
    "meta_harness.search_evaluator",
    "meta_harness.prompt_proposer",
    "meta_harness.ranking",
    "meta_harness.search_orchestrator",
    "meta_harness.winner_freeze",
    "meta_harness.final_evaluation",
    "meta_harness.server_run",
)
...
def test_all_supported_experiment_modules_import_offline():
    for module_name in CURRENT_MODULES:
        assert importlib.import_module(module_name).__name__ == module_name


def test_active_source_surface_has_no_versioned_module_or_cli_names():
    assert not list((REPOSITORY_ROOT / "meta_harness").glob("full_search_v3_*.py"))
    assert not (REPOSITORY_ROOT / "scripts" / "run_full_search_v3_server.py").exists()
    assert not (REPOSITORY_ROOT / "notebooks" / "sciver_full_search_v3_server.ipynb").exists()
...
def test_supported_documentation_has_no_removed_commands():
    supported_docs = (
        ...
        REPOSITORY_ROOT / "docs" / "sciver_meta_harness_operations.md",
    )
...
def test_canonical_server_notebook_is_the_only_tracked_notebook():
    ...
    assert notebooks == ["sciver_meta_harness.ipynb"]
...
def test_readme_commands_and_referenced_repository_paths_exist():
    ...
    for relative_path in (
        ...
        "docs/sciver_meta_harness_operations.md",
