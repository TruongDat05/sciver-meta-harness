"""Pure offline tests for locked full SEARCH V3 rank and patience semantics."""
...
def _report(candidate, *, rankable=True, complete=True):
    total = FULL_SEARCH_V3_SEARCH_SIZE if complete else 999
    return {
...
def test_macro_f1_has_priority_over_accuracy():
    ...
    ranking = rank_full_search_v3_candidates([higher_accuracy, higher_f1])
...
def test_accuracy_breaks_a_macro_f1_tie():
    ...
    assert rank_full_search_v3_candidates([lower_accuracy, higher_accuracy]) == (
        higher_accuracy,
        lower_accuracy,
    )
...
def test_prompt_sha256_breaks_an_exact_metric_tie():
    ...
    assert rank_full_search_v3_candidates([later_hash, earlier_hash]) == (
        earlier_hash,
        later_hash,
    )
...
def test_candidate_id_breaks_an_exact_hash_and_metric_tie():
    ...
    assert rank_full_search_v3_candidates([later_id, earlier_id]) == (
        earlier_id,
        later_id,
    )
...
def test_hash_or_id_only_winner_change_does_not_reset_patience(candidates):
    ...
    update = advance_full_search_v3_patience(state, candidates)
...
def test_true_metric_improvement_resets_patience():
    ...
    update = advance_full_search_v3_patience(state, [improved])
...
def test_canonical_p0_can_beat_all_worse_candidates():
    ...
    ranking = rank_eligible_full_search_v3_reports([_report(worse), _report(p0)])
...
def test_incomplete_candidate_is_ineligible_and_never_ranked():
    ...
    assert eligible_full_search_v3_candidate(_report(incomplete, complete=False)) is None
    assert rank_eligible_full_search_v3_reports(
...
def test_ranking_is_deterministic_independent_of_input_order():
    ...
    forward = rank_full_search_v3_candidates(candidates)
    backward = rank_full_search_v3_candidates(reversed(candidates))
