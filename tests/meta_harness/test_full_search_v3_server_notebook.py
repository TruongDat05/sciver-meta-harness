...
NOTEBOOK_PATH = REPOSITORY_ROOT / "notebooks" / "sciver_full_search_v3_server.ipynb"
...
def test_notebook_never_changes_checkout_or_installs_dependencies():
    ...
    assert "validate_full_search_v3_server_checkout" in source
...
def test_notebook_offline_smoke_precedes_any_credential_reader_call():
    source = _source(_notebook())
    offline_call = source.index("offline_smoke = preflight_full_search_v3_server_run(")
...
def test_readme_references_the_only_canonical_notebook():
    ...
    assert tracked_notebooks == ["notebooks/sciver_full_search_v3_server.ipynb"]
