...
import scripts.run_full_search_v3_server as cli_module
from meta_harness.full_search_v3_server import FullSearchV3ServerError
...
def test_live_search_delegates_once_to_m6_without_credential_arguments(monkeypatch):
    ...
    monkeypatch.setattr(cli_module, "start_or_resume_full_search_v3_server_run", fake_start)
    messages = []
...
def test_process_lock_error_is_safe_and_activity_delegates_to_m6(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (_ for _ in ()).throw(
            FullSearchV3ServerError("another process owns this V3 run")
        ),
    )
    errors = []
    assert cli_module.cli(_search_arguments("--live-search"), output=errors.append) == 2
    assert errors == ["error: another process owns this V3 run"]
...
def test_interruption_resume_uses_same_run_and_never_redispatches_completed_work(monkeypatch):
    ...
    def fake_start(**kwargs):
        ...
        if not interrupted:
            ...
            raise FullSearchV3ServerError("simulated infrastructure interruption")
