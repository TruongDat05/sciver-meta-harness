"""Thin, opt-in command wrapper for the M6 full-SEARCH V3 server interface.

This wrapper owns no experiment logic.  It prints only sanitized structured
status and delegates every operation to :mod:`meta_harness.full_search_v3_server`.
...
def cli(
    ...
    *,
    ...
    except FullSearchV3ServerError as exc:
...
def _dispatch(arguments: argparse.Namespace) -> Mapping[str, Any]:
    ...
    if arguments.operation == "prepare":
        return prepare_full_search_v3_server_run(
            **common,
    ...
    if arguments.operation == "search-preflight":
        return preflight_full_search_v3_server_run(
            **common,
    ...
    if arguments.operation == "search":
        ...
        return start_or_resume_full_search_v3_server_run(
            **common,
            search_safe_manifest_path=arguments.search_safe_manifest,
            search_records_path=arguments.search_records,
            source_commit=arguments.source_commit,
    ...
    if arguments.operation == "search-status":
        return inspect_full_search_v3_server_status(**common)
    if arguments.operation == "activity":
        return inspect_full_search_v3_server_activity(**common)
    if arguments.operation == "freeze":
        return freeze_full_search_v3_server_winner(**common)
    if arguments.operation == "final-preflight":
        return preflight_full_search_v3_server_final(
            **common,
            dataset_path=arguments.dataset_path,
    ...
    if arguments.operation == "final":
        ...
        return start_or_resume_full_search_v3_server_final(
            **common,
            dataset_path=arguments.dataset_path,
            private_manifest_path=arguments.private_manifest,
            search_safe_manifest_path=arguments.search_safe_manifest,
    ...
    if arguments.operation == "final-status":
        return inspect_full_search_v3_server_final_status(**common)
