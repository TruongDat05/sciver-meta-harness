...
class RequestExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(cache, SearchCache):
            raise TypeError("cache must be SearchCache")
    ...
    def complete_ordered(
        self,
        requests: Sequence[tuple[SolverRequestIdentity, SolverRequest]],
    ) -> Iterator[SolverResult | SolverExecutionFailure]:
        """Complete requests with bounded concurrency in input order.

        Transport work is kept full up to the configured limit even when an
        earlier request is slower than a later one. Results are nevertheless
        yielded only in manifest order so the evaluator remains the sole
        checkpoint writer.
        """
        ...
        next_result = 0
        exhausted = False
        in_flight: dict[Future[SolverResult], int] = {}
        completed: dict[int, Future[SolverResult]] = {}

        def submit_available(pool: ThreadPoolExecutor) -> None:
            nonlocal submitted, exhausted
            while not exhausted and len(in_flight) < self._maximum_in_flight_requests:
                try:
                    identity, request = next(request_iterator)
                except StopIteration:
                    exhausted = True
                    return
                in_flight[pool.submit(self.complete, identity, request)] = submitted
                submitted += 1

        with ThreadPoolExecutor(
            max_workers=self._maximum_in_flight_requests,
        ) as pool:
            submit_available(pool)
            while in_flight:
                done, _ = wait(in_flight, return_when=FIRST_COMPLETED)
                for future in done:
                    completed[in_flight.pop(future)] = future
                while next_result in completed:
                    future = completed.pop(next_result)
                    next_result += 1
                    try:
                        yield future.result()
                    except SolverExecutionFailure as error:
                        # A logical request failure is reportable per sample;
                        # later manifest entries must still be attempted.
                        yield error
                    # Resume after the evaluator durably checkpoints this
                    # manifest result. With one worker this preserves the
                    # original request/checkpoint interruption boundary.
                    submit_available(pool)
                # When a later result completed before the next manifest
                # result, refill the freed worker without waiting for that
                # slower earlier request.
                submit_available(pool)
