"""Strict data identity and paper-exclusive splitting for full SEARCH.
...
identities, and creates the two membership pools needed by later
preparation code.
"""
...
EXPERIMENT_RECORD_SCHEMA_VERSION = "sciver_source_record_v1"
EXPERIMENT_SAMPLE_IDENTITY_VERSION = "sciver_sample_content_v1"
EXPERIMENT_PAPER_IDENTITY_VERSION = "sciver_paper_reference_v1"
EXPERIMENT_SPLIT_SCHEMA_VERSION = "sciver_paper_split_v1"
EXPERIMENT_ALLOCATION_ALGORITHM = "seeded_exact_paper_dp_v1"
EXPERIMENT_POOLS = ("SEARCH", "FINAL")
...
class DataError(ValueError):
    """Raised when a SciVer source record is not safe for the contract."""
...
class ValidatedSciVerRecord:
    """A strictly checked source record with immutable identities."""
...
def validate_sciver_records(
    records: Sequence[Mapping[str, Any]], *, source_path: str | Path
) -> tuple[ValidatedSciVerRecord, ...]:
    """Check released SciVer records and return their stable identities.
    ...
    row-based IDs are not valid membership identities.
    """

    if isinstance(records, (str, bytes)) or not isinstance(records, Sequence):
        raise DataError("SciVer source records must be a sequence")
    resolved_source = Path(source_path).expanduser().resolve()
    if not resolved_source.is_file():
        raise DataError("SciVer source path must name an existing file")

    validated: list[ValidatedSciVerRecord] = []
    ...
    for index, record in enumerate(records):
        ...
        if prior_index is not None:
            raise DataError(
                "SciVer sample identity collision between source records "
...
def build_experiment_split(
    ...
    *,
    ...
    """Allocate exact pools by whole paper groups with seeded DP ties."""
    ...
    verify_experiment_split(result, records, config=active_config)
    return result


def verify_experiment_split(
    result: Mapping[str, Any],
    records: Sequence[ValidatedSciVerRecord],
    *,
    config: Config | None = None,
) -> None:
    """Raise a clear error unless a result is an exact paper split."""

    active_config = config or canonical_experiment_config()
    _require_v3_config(active_config)
    if not isinstance(result, Mapping):
        raise SplitError("Meta-Harness split result must be an object")
    expected_fields = {
        "schema_version",
        "protocol_id",
        "split_seed",
    ...
    _require_exact_result_fields(result, expected_fields, "Meta-Harness split result")
    locked_values = {
        "schema_version": EXPERIMENT_SPLIT_SCHEMA_VERSION,
    ...
    for field, expected in locked_values.items():
        ...
        if type(actual) is not type(expected) or actual != expected:
            raise SplitError(f"Meta-Harness split result has invalid {field}")
    ...
    if result["eligible_sample_count"] != len(sample_index):
        raise SplitError("Meta-Harness split eligible sample count does not match source")
    if result["eligible_paper_count"] != len(ordered_groups):
        raise SplitError("Meta-Harness split eligible paper count does not match source")
    if result["source_fingerprint"] != _source_fingerprint(sample_index):
        raise SplitError("Meta-Harness split source fingerprint does not match source")
    ...
    for pool_name, expected_size in (
        ...
        if not isinstance(pool, Mapping):
            raise SplitError(f"Meta-Harness {pool_name} pool must be an object")
        _require_exact_result_fields(
            pool,
            {"sample_ids", "paper_identities", "sample_count", "paper_count"},
            f"Meta-Harness {pool_name} pool",
        )
        sample_ids = _string_tuple(pool["sample_ids"], f"Meta-Harness {pool_name} sample IDs")
        paper_ids = _string_tuple(
            pool["paper_identities"], f"Meta-Harness {pool_name} paper identities"
        )
        if len(sample_ids) != len(set(sample_ids)):
            raise SplitError(f"Meta-Harness {pool_name} sample IDs are not unique")
        if len(paper_ids) != len(set(paper_ids)):
            raise SplitError(f"Meta-Harness {pool_name} paper identities are not unique")
        if pool["sample_count"] != expected_size or len(sample_ids) != expected_size:
            raise SplitError(
                f"Meta-Harness {pool_name} must contain exactly {expected_size} samples"
            )
        if pool["paper_count"] != len(paper_ids):
            raise SplitError(f"Meta-Harness {pool_name} paper count is invalid")
        if any(sample_id not in sample_index for sample_id in sample_ids):
            raise SplitError(
                f"Meta-Harness {pool_name} references a sample absent from validated source"
            )
        if any(paper_id not in {group.paper_identity for group in groups} for paper_id in paper_ids):
            raise SplitError(
                f"Meta-Harness {pool_name} references a paper absent from validated source"
            )
        ...
        if paper_ids != expected_papers:
            raise SplitError(
                f"Meta-Harness {pool_name} paper ordering is not deterministic"
            )
        ...
        if sample_ids != expected_samples:
            raise SplitError(
                f"Meta-Harness {pool_name} sample ordering or paper ownership is invalid"
            )
...
def _validate_one_record(
    ...
    if isinstance(record["request_id"], bool) or not isinstance(record["request_id"], int):
        raise DataError(f"{context} request_id must be an integer")
    if not isinstance(record["label"], bool):
        raise DataError(f"{context} label must be a JSON boolean")
    ...
    identity_payload = {
        "identity_version": EXPERIMENT_SAMPLE_IDENTITY_VERSION,
    ...
    sample_id = (
        f"{EXPERIMENT_SAMPLE_IDENTITY_VERSION}:"
        f"{_canonical_sha256(identity_payload)}"
    )
    return ValidatedSciVerRecord(
        sample_id=sample_id,
        paper_identity=paper_identity,
...
def _require_source_fields(
    ...
    *,
    ...
    if missing or unexpected:
        ...
        raise DataError(f"{context} has invalid fields ({'; '.join(details)})")
...
def _section_list(value: Any, context: str) -> list[str]:
    if not isinstance(value, list) or not value:
        raise DataError(f"{context} section must be a non-empty list")
    sections = list(value)
...
def _resolve_local_reference(value: str, source_path: Path, context: str) -> Path:
    if "://" in value or value.startswith("data:"):
        raise DataError(f"{context} must be a local path")
    ...
    for path in candidates:
        ...
        if resolved.is_file():
            return resolved
    raise DataError(f"{context} does not resolve to a local file")
...
def _read_paper_document(path: Path, context: str) -> tuple[bytes, Mapping[str, Any]]:
    try:
        ...
        raise DataError(f"{context} must reference a valid JSON document") from exc
    if not isinstance(document, Mapping):
        raise DataError(f"{context} paper JSON must be an object")
...
def _check_sections_exist(
    ...
    if isinstance(paper_sections, (str, bytes)) or not isinstance(paper_sections, Sequence):
        raise DataError(f"{context} paper JSON has no valid sections list")
    available: set[str] = set()
    requested = set(sections)
    for section in paper_sections:
        if not isinstance(section, Mapping):
            raise DataError(f"{context} paper JSON has a malformed section")
...
def _check_evidence(
    ...
    if evidence_type not in _EVIDENCE_TYPES:
        raise DataError(f"{context} type must be chart or table")
    if isinstance(item, bool) or not isinstance(item, (int, str)):
        raise DataError(f"{context} item must be an integer or string")
    ...
    if not isinstance(catalog, Mapping):
        raise DataError(f"{context} paper JSON has no valid {catalog_name}")
    entry = catalog.get(str(item))
    if not isinstance(entry, Mapping):
        raise DataError(f"{context} item is absent from paper {catalog_name}")
    ...
    try:
        ...
        raise DataError(f"{context} path cannot be read") from exc
    return {
        "type": evidence_type,
        "item": item,
...
def _validated_sample_index(
    records: Sequence[ValidatedSciVerRecord],
) -> dict[str, ValidatedSciVerRecord]:
    if isinstance(records, (str, bytes)) or not isinstance(records, Sequence):
        raise SplitError("Meta-Harness split source must be validated record sequence")
    result: dict[str, ValidatedSciVerRecord] = {}
    for index, record in enumerate(records):
        if not isinstance(record, ValidatedSciVerRecord):
            raise SplitError(
                f"Meta-Harness split source record {index} was not strictly checked"
            )
        if record.sample_id in result:
            raise SplitError("Meta-Harness split source contains duplicate sample IDs")
        result[record.sample_id] = record
    return result


def _paper_groups(
    sample_index: Mapping[str, ValidatedSciVerRecord],
) -> tuple[_PaperGroup, ...]:
...
def _allocate_paper_groups(
    ...
    if not (reachable[target_total] & (1 << search_size)):
        ...
        raise SplitError(
            "cannot allocate exact paper-exclusive pools: "
    ...
    while current_state:
        ...
        if group_index < 0 or assignment not in {1, 2} or previous_state < 0:
            raise SplitError("Meta-Harness exact allocator has an invalid DP path")
        if assignment == 1:
...
def _source_fingerprint(sample_index: Mapping[str, ValidatedSciVerRecord]) -> str:
    payload = [
...
def _json_clone(value: Mapping[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise DataError("SciVer source record must contain JSON values") from exc
    if not isinstance(cloned, dict):
        raise DataError("SciVer source record must be a JSON object")
    return cloned


def _require_v3_config(config: Config) -> None:
    if not isinstance(config, Config):
        raise SplitError("Meta-Harness split requires Config")
    if config.protocol_id != EXPERIMENT_PROTOCOL_ID:
        raise SplitError("Meta-Harness split has an invalid protocol identity")
...
def _require_exact_result_fields(
    ...
    if actual != expected:
        ...
        raise SplitError(f"{context} fields are invalid ({'; '.join(details)})")


def _string_tuple(value: Any, context: str) -> tuple[str, ...]:
    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        raise SplitError(f"{context} must be a list")
    values = tuple(value)
    if any(not isinstance(item, str) or not item for item in values):
        raise SplitError(f"{context} must contain non-empty text")
    return values
