"""Trusted, resumable full-SEARCH V3 orchestration without FINAL access."""
...
FULL_SEARCH_V3_ORCHESTRATOR_VERSION = "sciver_full_search_v3_orchestrator_v1"
...
class FullSearchV3OrchestrationError(RuntimeError):
    """Raised when the V3 SEARCH state machine cannot advance safely."""


class FullSearchV3ResumeError(FullSearchV3OrchestrationError):
...
class FullSearchV3Orchestrator:
    """Run P0 then one fully evaluated candidate per locked V3 iteration.
    ...
    def __init__(
        ...
        *,
        ...
        p0_evaluator: P0Evaluator = evaluate_full_search_v3_p0,
        candidate_evaluator: CandidateEvaluator = evaluate_full_search_v3_candidate,
        ...
        if not isinstance(search_input, FullSearchV3SearchInput):
            raise TypeError("search_input must be FullSearchV3SearchInput")
        ...
        if not callable(p0_evaluator) or not callable(candidate_evaluator):
            raise TypeError("V3 evaluators must be callable")
        ...
        self.representative_search_failures = tuple(
            build_full_search_v3_proposer_input(
                iteration=0,
                parent_id=FULL_SEARCH_V3_P0_CANDIDATE_ID,
    ...
    def run(self) -> dict[str, Any]:
        """Advance until a durable V3 terminal state or retryable interruption."""
    ...
    def _ensure_p0(self) -> bool:
        ...
        except FullSearchV3EvaluationIncomplete:
            p0["status"] = "incomplete"
            self._persist("p0_incomplete")
            return False
        p0["report"] = _safe_report(report, FULL_SEARCH_V3_P0_CANDIDATE_ID)
        eligible = eligible_full_search_v3_candidate(p0["report"])
        if eligible is None:
    ...
    def _recover_or_propose(self, iteration: int) -> FullSearchV3ProposalResult | None:
        ...
        except FullSearchV3ProposalExhausted:
            entry = _iteration_entry(self._state, iteration)
    ...
    def _recover_accepted_proposal(
        self, iteration: int
    ) -> FullSearchV3ProposalResult | None:
        ...
        if len(accepted) != 1 or accepted[0].candidate.candidate_id in self._existing_candidate_ids():
            raise FullSearchV3OrchestrationError(
                "proposal receipts are incompatible with resumable V3 state"
            )
        return accepted[0]
    ...
    def _apply_completed_iteration(self, entry: Mapping[str, Any]) -> None:
        ...
        ranking = rank_eligible_full_search_v3_reports(reports)
        previous = FullSearchV3PatienceState(**self._state["patience"])
        update = advance_full_search_v3_patience(previous, ranking)
    ...
    def _parent_for_next_proposal(self) -> tuple[str, Mapping[str, str]]:
        ...
        if winner_id == FULL_SEARCH_V3_P0_CANDIDATE_ID:
        ...
        raise FullSearchV3OrchestrationError(
            "V3 ranking winner has no durable prompt family"
        )
    ...
    def _aggregate_search_metrics(self) -> dict[str, Any]:
        ...
        raise FullSearchV3OrchestrationError("V3 winner metrics are unavailable")
    ...
    def _existing_candidate_ids(self) -> list[str]:
        return [FULL_SEARCH_V3_P0_CANDIDATE_ID] + [
    ...
    def _existing_prompt_hashes(self) -> list[str]:
        values = [canonical_full_search_v3_p0_prompt_sha256()]
        values.extend(
    ...
    def _load_or_initialize_state(self) -> dict[str, Any]:
        if not self.state_path.exists():
            state = {
                "schema_version": FULL_SEARCH_V3_ORCHESTRATOR_SCHEMA_VERSION,
                "orchestrator_version": FULL_SEARCH_V3_ORCHESTRATOR_VERSION,
                "identity": self.identity,
                "status": "running",
                "stop_reason": None,
                "p0": {"status": "pending", "report": None},
                "iterations": [],
                "patience": _patience_dict(FullSearchV3PatienceState()),
                "ranking": [],
                "winner_id": None,
            }
        ...
        if state.get("schema_version") != FULL_SEARCH_V3_ORCHESTRATOR_SCHEMA_VERSION:
            raise FullSearchV3ResumeError("V3 orchestration state schema is incompatible")
...
def _run_identity(
    *,
    ...
    if not isinstance(manifest, Mapping) or any(name not in manifest for name in required):
        raise FullSearchV3OrchestrationError("SEARCH input lacks immutable manifest identity")
    return {
...
def _config_sha256() -> str:
    ...
    return canonical_full_search_v3_config().sha256()
...
def _safe_report(report: Mapping[str, Any], expected_candidate_id: str) -> dict[str, Any]:
    if not isinstance(report, Mapping) or report.get("candidate_id") != expected_candidate_id:
        raise FullSearchV3OrchestrationError("V3 evaluator returned an incompatible report")
    metrics = report.get("metrics")
    if not isinstance(metrics, Mapping):
        raise FullSearchV3OrchestrationError("V3 evaluator report lacks metrics")
    fields = (
        "protocol_id",
        "stage",
        "candidate_id",
    ...
    if any(field not in report for field in fields) or any(
        ...
        raise FullSearchV3OrchestrationError("V3 evaluator report is incomplete")
    return {
...
def _candidate_from_state(value: Any) -> FullSearchV3Candidate:
    if not isinstance(value, Mapping) or set(value) != {
        "candidate_id", "parent_id", "hypothesis", "expected_tradeoff", "templates", "source_sha256"
    }:
        raise FullSearchV3ResumeError("stored V3 candidate is invalid")
    ...
    try:
        ...
        raise FullSearchV3ResumeError("stored V3 candidate templates are invalid") from exc
    if source_sha256 != candidate.source_sha256:
        raise FullSearchV3ResumeError("stored V3 candidate hash is invalid")
    return candidate
...
def _patience_dict(state: FullSearchV3PatienceState) -> dict[str, Any]:
    return {
...
def _validate_state_shape(state: Mapping[str, Any]) -> None:
    ...
    if not isinstance(state, Mapping) or set(state) != required:
        raise FullSearchV3ResumeError("V3 orchestration state fields are invalid")
    if not isinstance(state["iterations"], list):
        raise FullSearchV3ResumeError("V3 orchestration iterations are invalid")
    if not isinstance(state["p0"], Mapping) or set(state["p0"]) != {"status", "report"}:
        raise FullSearchV3ResumeError("V3 orchestration P0 state is invalid")
    if state["p0"]["status"] not in {"pending", "evaluating", "incomplete", "complete", "ineligible"}:
        raise FullSearchV3ResumeError("V3 orchestration P0 status is invalid")
    ...
    if observed != expected:
        raise FullSearchV3ResumeError("V3 orchestration iteration order is invalid")
    for entry in state["iterations"]:
        if not isinstance(entry, Mapping) or set(entry) != {
            "iteration", "status", "candidate", "report", "metric_improved"
        }:
            raise FullSearchV3ResumeError("V3 orchestration iteration state is invalid")
        if entry["status"] not in {
            "proposal_pending", "proposal_exhausted", "proposed", "evaluating", "incomplete", "complete"
        }:
            raise FullSearchV3ResumeError("V3 orchestration iteration status is invalid")
        ...
        if entry["status"] == "complete" and not isinstance(entry["report"], Mapping):
            raise FullSearchV3ResumeError("completed V3 iteration lacks a report")
    FullSearchV3PatienceState(**state["patience"])
...
def _load_state(path: Path) -> dict[str, Any]:
    ...
    except (OSError, UnicodeError, json.JSONDecodeError, FullSearchV3ResumeError) as exc:
        raise FullSearchV3ResumeError("V3 orchestration state is unreadable") from exc
    if not isinstance(value, dict) or encoded != canonical_json(value).encode("utf-8"):
        raise FullSearchV3ResumeError("V3 orchestration state is not canonical")
    return value
...
def _run_lock(path: Path) -> Iterator[None]:
    ...
    try:
        try:
            ...
            raise FullSearchV3OrchestrationError("another process owns this V3 run") from exc
        yield
    finally:
...
def _unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise FullSearchV3ResumeError("duplicate V3 state JSON key")
        result[key] = value
    return result
