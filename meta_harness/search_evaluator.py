"""Immutable input boundary for the full SEARCH evaluator.
...
EXPERIMENT_REQUIRED_SPLIT_SHA256 = (
    "f45dff73f955e7f7c17740a1d061c88c8f8eb1d767802a0fba3e96913753e87a"
)
...
EXPERIMENT_P0_CANDIDATE_ID = "cot"
...
class PredictionOutcome:
    ...
    def from_solver_result(
        ...
        if not isinstance(result, SolverResult):
            raise EvaluatorInputError("result must be a SolverResult")
...
def load_experiment_search_input(
    *,
    ...
    try:
        manifest = load_experiment_search_safe_manifest(search_safe_manifest_path)
        records = load_experiment_search_dataset(search_records_path)
    except PreparationError as exc:
        raise EvaluatorInputError(
            "full SEARCH evaluator inputs must be valid prepared SEARCH artifacts"
        ) from exc
    return validate_experiment_search_input(manifest=manifest, records=records)


def validate_experiment_search_input(
    *,
    manifest: Mapping[str, Any],
    records: Sequence[Mapping[str, Any]],
) -> SearchInput:
    """Validate and freeze one complete SEARCH manifest/artifact pair."""

    if not isinstance(manifest, Mapping):
        raise EvaluatorInputError("SEARCH manifest must be an object")
    if manifest.get("protocol_id") != EXPERIMENT_PROTOCOL_ID:
        raise EvaluatorInputError(
            "SEARCH manifest does not identify the locked protocol"
        )
    if manifest.get("split_sha256") != EXPERIMENT_REQUIRED_SPLIT_SHA256:
        raise EvaluatorInputError(
            "SEARCH manifest does not use the required split SHA-256"
        )
    if "FINAL" in manifest or manifest.get("stage", EXPERIMENT_STAGE) != (
        EXPERIMENT_STAGE
    ):
        raise EvaluatorInputError(
    ...
    expected_count = canonical_experiment_config().search_size
    ...
    if len(observed_ids) != len(set(observed_ids)):
        raise EvaluatorInputError("SEARCH records contain duplicate sample IDs")
    ...
    try:
        ...
        verify_experiment_search_dataset(records, manifest)
    except PreparationError as exc:
        raise EvaluatorInputError(
...
def _require_complete_search_input(
    ...
    """Revalidate the only input eligible for a protocol artifact."""

    if not isinstance(search_input, SearchInput):
        raise EvaluatorInputError(
            "search_input must be a validated SearchInput"
        )
    ...
    return validate_experiment_search_input(
...
def _compute_prediction_metrics(
    ...
    stage, candidate ID, or eligibility/rankability field. Public reports
    must instead enter through :func:`account_experiment_predictions`.
    """

    if not isinstance(search_input, SearchInput):
        raise EvaluatorInputError(
            "search_input must be SearchInput"
        )
...
def account_experiment_predictions(
    *,
    search_input: SearchInput,
    candidate_id: str,
    outcomes: Sequence[PredictionOutcome],
    ...
    if metrics_path is not None:
        _persist_experiment_prediction_metrics(metrics_path, report)
    return report


def evaluate_experiment_candidate(
    *,
    search_input: SearchInput,
    candidate_id: str,
    ...
    if not isinstance(cache, SearchCache):
        raise EvaluatorInputError("cache must be SearchCache")
    if not isinstance(executor, RequestExecutor):
        raise EvaluatorInputError(
            "executor must be RequestExecutor"
        )
    ...
    for item in requests:
        ...
        if saved is not None:
            ...
            outcomes[sample_id] = PredictionOutcome.from_solver_result(
                sample_id, result
            )
    pending = tuple(
        item for item in requests if item["sample_id"] not in completed
    )
    pending_results = executor.complete_ordered(
        tuple((item["identity"], item["request"]) for item in pending)
    )
    for item in pending:
        sample_id = item["sample_id"]
        request_identity = item["identity"]
    ...
    ordered_outcomes = tuple(
        ...
        or PredictionOutcome(
            sample_id=item["sample_id"],
            infrastructure_failure=failures[item["sample_id"]],
        )
        for item in requests
    )
    report = account_experiment_predictions(
        search_input=search_input,
        candidate_id=candidate_id,
    ...
    if report["infrastructure_failures"]:
        ...
        _write_checkpoint(checkpoint_destination, checkpoint)
        raise EvaluationIncomplete(report)
    ...
    checkpoint["last_infrastructure_failures"] = []
    _write_checkpoint(checkpoint_destination, checkpoint)
    _persist_experiment_prediction_metrics(result_destination, report)
    return report


def canonical_experiment_p0_prompt_sha256() -> str:
    """Return the immutable source hash of the canonical ``cot`` P0 prompt."""

    return template_source_sha256(COT_PROMPT)


def evaluate_experiment_p0(
    *,
    search_input: SearchInput,
    solver_identity_sha256: str,
    cache: SearchCache,
...
def _validate_checkpoint(payload: Any, identity: Mapping[str, Any]) -> None:
    ...
    if (
        payload["schema_version"] != EXPERIMENT_CHECKPOINT_SCHEMA_VERSION
    ...
    if payload["status"] not in {"running", "incomplete", "ineligible", "eligible"}:
        raise EvaluationResumeError("full SEARCH checkpoint status is invalid")
...
def _rehydrate_completed_result(
    ...
    outcome = PredictionOutcome.from_solver_result(
...
def _completed_entry(identity: Any, outcome: PredictionOutcome) -> dict[str, Any]:
...
def _validate_final_report(
    ...
    *,
    ...
    if (
        report.get("stage") != EXPERIMENT_STAGE
...
def _atomic_replace_bytes(path: Path, encoded: bytes, context: str) -> None:
    ...
    try:
        ...
        raise EvaluatorInputError(f"{context} cannot be created") from exc
    ...
    try:
        ...
        raise EvaluatorInputError(f"{context} cannot be persisted") from exc
    finally:
        temporary.unlink(missing_ok=True)


def _persist_experiment_prediction_metrics(
    ...
    try:
        ...
        try:
            ...
            return _persist_experiment_prediction_metrics(destination, report)
        except OSError as exc:
            raise EvaluatorInputError(
...
def _validate_outcomes(
    ...
    if isinstance(outcomes, (str, bytes)) or not isinstance(outcomes, Sequence):
        raise EvaluatorInputError("prediction outcomes must be a sequence")
    ordered = tuple(outcomes)
    expected_ids = search_input.sample_ids
    observed_ids = tuple(
        outcome.sample_id
        if isinstance(outcome, PredictionOutcome)
        else None
        for outcome in ordered
    )
    if any(sample_id is None for sample_id in observed_ids):
        raise EvaluatorInputError(
            "prediction outcomes must use PredictionOutcome"
        )
    ...
    if len(set(observed_ids)) != len(observed_ids):
        raise EvaluatorInputError("prediction outcomes contain duplicates")
    if set(observed_ids) != set(expected_ids):
        raise EvaluatorInputError(
...
def _manifest_sample_ids(manifest: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(search, Mapping):
        raise EvaluatorInputError("SEARCH manifest has no SEARCH stage")
    sample_ids = search.get("sample_ids")
    if isinstance(sample_ids, (str, bytes)) or not isinstance(sample_ids, Sequence):
        raise EvaluatorInputError("SEARCH manifest sample IDs must be a list")
...
def _record_sample_ids(records: Sequence[Mapping[str, Any]]) -> tuple[str, ...]:
    if isinstance(records, (str, bytes)) or not isinstance(records, Sequence):
        raise EvaluatorInputError("SEARCH records must be a sequence")
    sample_ids: list[str] = []
