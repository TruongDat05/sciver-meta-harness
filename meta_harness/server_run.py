...
from meta_harness.solver import create_live_solver_client
...
def validate_server_run_checkout(
    *,
    ...
    if not root.is_dir() or not (root / ".git").exists():
        raise ServerError("repository root is not a Git checkout")
    if (
    ...
    if expected_origin_url != EXPECTED_REPOSITORY_ORIGIN:
        raise ServerError("expected repository origin is not canonical")

    top_level = _git_output(root, "rev-parse", "--show-toplevel")
    try:
        resolved_top_level = Path(top_level).resolve()
    except (OSError, RuntimeError) as exc:
        raise ServerError("repository top level is invalid") from exc
    if resolved_top_level != root:
        raise ServerError("repository root does not match the Git top level")
    origin = _git_output(root, "remote", "get-url", "origin")
    if origin != expected_origin_url:
        raise ServerError("repository origin does not match the expected URL")
    head = _git_output(root, "rev-parse", "HEAD")
    if head != pinned_commit_sha:
        raise ServerError("checked-out commit does not match PINNED_COMMIT_SHA")
    status = _git_output(
        root, "status", "--porcelain=v1", "--untracked-files=all", allow_empty=True
    )
    # The thin operator notebook self-saves autogenerated cell output and
    # kernel metadata on every run, so it would otherwise always dirty its own
    # working tree. It is independently validated to be output-free at commit
    # time, so exclude it (and its Jupyter checkpoint file) from the
    # cleanliness guard; every other source change still fails closed.
    operator_source_tokens = ("sciver_meta_harness.ipynb", ".ipynb_checkpoints/")
    status = "\n".join(
        line
        for line in status.splitlines()
        if not any(token in line for token in operator_source_tokens)
    )
    if status:
        raise ServerError(
    ...
    required_files = (
        ...
        root / "notebooks" / "sciver_meta_harness.ipynb",
        root / "meta_harness" / "server_run.py",
    )
    if any(not path.is_file() for path in required_files):
        raise ServerError("repository deployment metadata is incomplete")
    return {
...
def preflight_search_run(
    *,
    ...
    search_input = load_experiment_search_input(
    ...
    active_proposer_config = proposer_config or ProposerConfig()
    ...
    state_path = experiment_orchestration_state_path(repository_root, safe_run_id)
...
def start_or_resume_search_run(
    *,
    repository_root: str | Path,
    run_id: str,
    search_safe_manifest_path: str | Path,
    ...
    active_proposer_config = proposer_config or ProposerConfig()
    preflight = preflight_search_run(
        repository_root=repository_root,
        run_id=run_id,
        search_safe_manifest_path=search_safe_manifest_path,
        search_records_path=search_records_path,
    ...
    run_directory = _run_directory(repository_root, run_id)
    cache = SearchCache(run_directory / "search_cache")
    executor = RequestExecutor(
        cache=cache,
        client_factory=lambda: _construct_live_solver(runtime_url, runtime_key),
    ...
    state = Orchestrator(
        ...
        cache=cache,
        executor=executor,
        proposer=Proposer(config=active_proposer_config),
...
def run_meta_harness_smoke(
    *,
    ...
    receipt_path = server_run_smoke_receipt_path(repository_root, run_id)
...
def server_run_smoke_receipt_path(
...
def preflight_server_run_final(
    *,
    repository_root: str | Path,
    run_id: str,
    dataset_path: str | Path,
    ...
    if solver_identity_sha256 is None:
        state_path = experiment_orchestration_state_path(repository_root, run_id)
        state = load_experiment_orchestration_state(state_path)
        identity = state.get("identity")
        if not isinstance(identity, Mapping):
            raise ServerError("SEARCH run identity is missing")
    ...
    return preflight_final_evaluation(
        repository_root=repository_root,
        run_id=run_id,
        ...
        solver_contract=canonical_final_evaluation_solver_contract(
            solver_identity_sha256=solver_identity_sha256
        ),
    )


def start_or_resume_server_run_final(
    *,
    repository_root: str | Path,
    run_id: str,
    dataset_path: str | Path,
    ...
    preflight = preflight_final_evaluation(
        repository_root=repository_root,
        run_id=run_id,
        frozen_winner_path=None,
        private_manifest_path=private_manifest_path,
...
def inspect_server_run_activity(
    *, repository_root: str | Path, run_id: str
) -> dict[str, Any]:
...
def solver_identity_from_api_url(
    ...
    *,
    ...
    try:
        ...
        raise ServerError("runtime API_URL is not a safe base URL") from exc
...
def _preparation_status(
    run_id: str, run_directory: Path, artifacts: PreparationArtifacts
) -> dict[str, Any]:
    return {
        "operation": "prepare",
        "run_id": _run_id(run_id),
    ...
    return {
        ...
        "checkpoints": {
            "smoke_receipt_path": str(
                server_run_smoke_receipt_path(repository_root, run_id)
            ),
            "search_state_path": str(
                experiment_orchestration_state_path(repository_root, run_id)
            ),
            "search_cache_directory": str(
                _run_directory(repository_root, run_id) / "search_cache"
            ),
            "frozen_winner_path": str(
                experiment_frozen_winner_path(repository_root, run_id)
            ),
            "final_state_path": str(
                final_evaluation_state_path(repository_root, run_id)
            ),
            "final_receipt_path": str(
                final_evaluation_completion_receipt_path(repository_root, run_id)
            ),
        },
        "resume": resume,
        "resume_identity_checked": resume_identity_checked,
...
def _smoke_identity_and_request(
    preflight: Mapping[str, Any], search_input: SearchInput
) -> tuple[dict[str, Any], Any]:
    if not search_input.records:
        raise ServerError("SMOKE requires at least one validated SEARCH record")
...
def _trusted_final_records(
    ...
    try:
        ...
        raise ServerError("dataset must be readable valid JSON") from exc
    if not isinstance(raw_records, list) or any(
        not isinstance(record, Mapping) for record in raw_records
    ):
        raise ServerError("dataset must be a JSON list of objects")
    try:
        return materialize_final_evaluation_records(
            validate_sciver_records(raw_records, source_path=source),
            load_trusted_experiment_private_manifest(private_manifest_path),
            source_path=source,
        )
...
def _construct_live_solver(api_url: str, api_key: str) -> Any:
    # SEARCH workers create clients lazily and concurrently.  The legacy
    # client factory reads runtime configuration from process environment, so
    # serialize its short-lived environment substitution while still returning
    # a distinct client to each worker.
...
def _runtime_credentials(*, api_url: str | None, api_key: str | None) -> tuple[str, str]:
    ...
    if not isinstance(runtime_url, str) or not runtime_url.strip():
        raise ServerError("live execution requires runtime API_URL")
    if not isinstance(runtime_key, str) or not runtime_key.strip():
        raise ServerError("live execution requires runtime API_KEY")
    if "\r" in runtime_key or "\n" in runtime_key:
        raise ServerError("runtime API_KEY contains invalid characters")
    try:
...
def _transport_contract_identity(
    *,
    ...
    return {
        "version": EXPERIMENT_TRANSPORT_CONTRACT_VERSION,
...
def _run_model_list_preflight(client: Any) -> dict[str, Any]:
    try:
        ...
        if "immutable solver model" in str(exc):
...
def _read_smoke_receipt(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(receipt, dict):
        raise ServerError("SMOKE receipt must be a JSON object")
    return receipt
...
def _valid_model_list_preflight(value: Any) -> bool:
    ...
    return (
        ...
        and value.get("schema_version") == EXPERIMENT_MODEL_LIST_PREFLIGHT_VERSION
...
def _proposer_identity(
    source_commit: str, config: ProposerConfig
) -> dict[str, Any]:
    return {
        "kind": "local_cli",
...
def _git_output(
    ...
    *arguments: str,
    ...
    try:
        ...
        raise ServerError("repository identity validation failed") from exc
    value = completed.stdout.strip()
    if not value and not allow_empty:
        raise ServerError("repository identity validation returned no value")
    return value
...
def _source_commit(repository_root: str | Path, supplied: str | None) -> str:
    if supplied is not None:
        if not isinstance(supplied, str) or not _COMMIT.fullmatch(supplied):
            raise ServerError("source_commit must be a lowercase Git commit hash")
    try:
        completed = subprocess.run(
            ["git", "rev-parse", "HEAD"],
            cwd=repository_root,
...
def _run_directory(repository_root: str | Path, run_id: str) -> Path:
    return experiment_orchestration_state_path(repository_root, _run_id(run_id)).parent
...
def _require_sha256(value: str, field: str) -> None:
    if not isinstance(value, str) or not _SHA256.fullmatch(value):
        raise ServerError(f"{field} must be a lowercase SHA-256")
