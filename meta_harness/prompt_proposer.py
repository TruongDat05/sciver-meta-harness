...
EXPERIMENT_PROPOSER_SCHEMA_VERSION = 1
EXPERIMENT_PROPOSER_INSTRUCTION_VERSION = "sciver_full_search_v3_proposer_v1"
...
def load_experiment_accepted_proposal(
    ...
    *,
    ...
    except (OSError, UnicodeError, json.JSONDecodeError, CandidateValidationError) as exc:
        raise ProposerInfrastructureError(
    ...
    if (
        ...
        raise ProposerInfrastructureError(
            "proposal receipt is not an accepted proposal"
        )
    ...
    except (InvalidPromptFamilyError, CandidateValidationError) as exc:
        raise ProposerInfrastructureError(
...
def _validate_candidate_templates(
    ...
    if not isinstance(value, Mapping):
        raise CandidateValidationError("candidate templates must be an object")
    try:
...
def _reject_duplicate(
    ...
    if candidate.candidate_id in existing_ids:
        raise CandidateValidationError("candidate ID duplicates an earlier proposal")
...
def _resumable_attempt_state(
    ...
    *,
    ...
    for expected_attempt, path in enumerate(receipts, start=1):
        ...
        if status == "rejected" and category in {"invalid_output", "duplicate"}:
            rejected_attempts += 1
...
def _safe_candidate_text(value: Any, field: str) -> str:
    ...
    if len(value) > 2_000 or _SENSITIVE_TEXT.search(value):
        raise CandidateValidationError(f"{field} contains prohibited data")
    return value.strip()
...
def _identifier(value: Any, field: str, pattern: re.Pattern[str] = _IDENTIFIER) -> str:
    if not isinstance(value, str) or not pattern.fullmatch(value):
        raise CandidateValidationError(f"{field} must be a safe identifier")
    return value
...
def _sha256(value: Any, field: str) -> str:
    if not isinstance(value, str) or not _SHA256.fullmatch(value):
        raise CandidateValidationError(f"{field} must be a SHA-256")
    return value
